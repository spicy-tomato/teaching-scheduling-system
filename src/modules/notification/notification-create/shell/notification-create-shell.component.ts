  ...
  implements OnInit
{
  ...
  @ViewChild('successDialog', { static: true })
  public successDialog!: PolymorpheusContent<TuiDialogContext>;
  //#endregion
  ...
  private handleSubmit(): void {
    this.confirm$
      .pipe(
        debounceTime(300),
        tap(() => {
          const value: NotificationCreateFormModel = this.form
            .value as NotificationCreateFormModel;

          const errors = Object.keys(this.form.controls).reduce(
          ...
          this.store.dispatch(
            fromNotificationCreate.clickConfirm({ value, errors })
          );
        }),
  ...
  private handleValidForm(): void {
    this.status$
      .pipe(
        ...
        mergeMap(() => this.dialogService.open(this.successDialog)),
        takeUntil(this.destroy$)
      )
      .subscribe();
  }
